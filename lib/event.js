...
exports.agreeGroupInvite = function agreeGroupInvite(data, cb) {
  let app = this.app;
  let socket = this.socket;
  ...
  try {
    ...
    if(!!player) {
      ...
      app.storage.connect(function(db) {
        ...
        Invite.one({uuid: inviteUUID, to_uuid: playerUUID}, function(err, invite) {
          if(!!err) {
            debug('agreeGroupInvite failed: %O', err);
            cb({result: false, msg: '同意失败'});
          }else {
            invite.is_agree = true;
            invite.save(function (err) {
              if(!!err) {
                debug('agreeGroupInvite save data failed: %O', err);
                cb({result: false, msg: '同意失败'});
              }else {
                // 设定群组关系
                let groupUUID = invite.group_uuid;
                app.group.addGroup(groupUUID, playerUUID, function(err) {
                  if(!!err) {
