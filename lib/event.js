...
exports.findGroup = async function findGroup(data, cb) {
  ...
  try {
    ...
    if(type === 'uuid') {
      ...
      results = await db.models.group_group.find().limit(10).where(`name like %${text}%`).allAsync();
    }else if(type === 'groupdesc') {
      results = await db.models.group_group.find().limit(10).where(`desc like %${text}%`).allAsync();
    }
