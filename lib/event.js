...
exports.agreeGroupRequest = async function agreeGroupRequest(data, cb) {
  let app = this.app;
  let socket = this.socket;
  ...
  try {
    ...
    cb({result: true, groupUUID: group.uuid, members: members_uuid});
    db.close();
  }catch(e) {
    cb({result: false, msg: e.toString()})
  }
...
exports.tickMember = async function tickMember(data, cb) {
  ...
  try {
    ...
    // 发通知
    app.chat.sendSystemMsg(memberUUID, 'msg', '', `您已被踢出团 ${group.name}`);
    group.getManagerUUIDs().forEach(uuid => {
      app.chat.sendSystemMsg(uuid, 'msg', '', `团成员 ${member.getName()} 已被踢出团 ${group.name}`);
    })
    cb({result: true});
    db.close();
  }catch(e) {
...
exports.setMemberToManager = async function setMemberToManager(data, cb) {
  ...
  try {
    ...
    // 发通知
    app.chat.sendSystemMsg(memberUUID, 'msg', '', `您已成为团 ${group.name} 的管理员`);
    group.getManagerUUIDs().forEach(uuid => {
      app.chat.sendSystemMsg(uuid, 'msg', '', `团成员 ${member.getName()} 已被提升为团 ${group.name} 的管理员`);
    })
